# Chronus Portfolio Product Context

## Register

Brand. This is a personal portfolio where design is part of the signal: it should make Jon Marien feel precise, security-minded, technical, and memorable without turning into a cyberpunk template.

## Audience

- Recruiters, founders, security teams, and engineering leads scanning for proof of skill.
- Technical peers who may click through to GitHub, security writeups, D-Sports, BearHacks, or the Chrono blog.
- Event and community partners who need to understand the scope of hackathon and cybersecurity work quickly.

## Purpose

Present the `chrono` identity through a compact terminal/editor portfolio that proves security research, full-stack shipping ability, and community leadership. The page should make the important evidence easy to scan: CVEs and labs, flagship projects, live products, real metrics, and reliable contact paths.

## Brand Personality

Terminal-native, exacting, quietly intense. The voice should be direct and evidence-led: fewer claims, more artifacts.

## Identity Anchors

- `~/chrono`, `whoami`, hash-section headers, and path-like breadcrumbs.
- Dark editor surfaces with restrained purple and teal accents.
- Monospace chrome, concise command-like labels, and real project/security/community assets.
- The ASCII `// chrono` hero is the signature visual. Keep it readable, performant, and respectful of reduced-motion preferences.

## Anti-References

- Generic cyber glass, RGB gamer glow, stock hacker imagery, and decorative neon noise.
- Resume paragraphs compressed into walls of claims.
- Fake stats, fake testimonials, generic avatars, and decorative icon spam.
- Purple gradients used as atmosphere instead of meaning.
- Controls that all look equally important.

## Design Principles

1. Evidence before atmosphere: every visual element should support a real project, security finding, community event, or contact path.
2. Sparse terminal, not sci-fi dashboard: use restraint, sharp hierarchy, and readable density.
3. Accents are signals: purple and teal should indicate state, focus, category, or proof, not coat the whole page.
4. Interactions are utilities: sorting, details, media previews, and navigation must feel fast, deliberate, and keyboard-visible.
5. Preserve the compact editor shell while making the security and project evidence feel more intentional.
